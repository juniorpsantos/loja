<aside class="col-lg-3 order-lg-first">
    <div class="sidebar sidebar-shop">
        ...
        <div class="widget widget-cats">
            ...
            <ul>
                <?php
                $lerCategoriasOfertas = new Ler();
                $lerCategoriasOfertas->Leitura('categorias', "WHERE tipo = 'pai' ");
                $subDepartamentosSite = Formata::Resultado($lerCategoriasOfertas);
                if ($subDepartamentosSite):
                    foreach ($lerCategoriasOfertas->getResultado() as $subDepartameto):
                        $subDepartameto = (object) $subDepartameto;

                        $lerCategoriasOfertas->Leitura('produto', "WHERE id_categoria = :id", "id={$subDepartameto->id}");
                        $contaProdutos = Formata::Resultado($lerCategoriasOfertas);
                        if ($contaProdutos):
                            $contadorDeProdutos = $lerCategoriasOfertas->getContaLinhas();
                        else:
                            $contadorDeProdutos = 0;
                        endif;
                ?>
                        <li><a href="<?= HOME . '/categorias/' . $subDepartameto->id . '/' .  $subDepartameto->url ?>" title="<?= $subDepartameto->nome ?>"><?= $subDepartameto->nome ?><span><?= $contadorDeProdutos ?></span></a></li>
                <?php
                    endforeach;
                endif;
                ?>
        ...
        <div class="sidebar sidebar-product">
            <div class="widget widget-products">
                ...
                <div class="products">
                    <?php
                    $lerCategoriasOfertas->Leitura('produto', "WHERE id_sub_categoria = :id ORDER BY data DESC LIMIT 3", "id={$subDepartameto->id}");
                    $produtos = Formata::Resultado($lerCategoriasOfertas);
                    if ($produtos):
                        foreach ($lerCategoriasOfertas->getResultado() as $produto):
