<div class="main-content">
    ...
    $estado = filter_input_array(INPUT_POST,  FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    if (isset($estado['sendZion'])) {
        unset($estado['sendZion']);

        if ($estado['zion_firewall'] != $_SESSION['_zion_firewall']) {
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-estados/index&erro=true&token=" . $_SESSION['timeWT']);
            exit();
        }

        $salvar = new Estados();
        $salvar->inserirEstado($estado);
