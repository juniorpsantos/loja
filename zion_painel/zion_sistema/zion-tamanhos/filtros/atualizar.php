<div class="main-content">
    ...
    if (isset($atualizar['sendZion'])) {
        ...
        if ($atualizar['zion_firewall'] != $_SESSION['_zion_firewall']) {
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-tamanhos/index&erro=true&token={$_SESSION['timeWT']}");
            exit();
        }

        $salvar = new Adicionais();
        $salvar->atualizaAdicional($atualizar['id'], $atualizar);
        if ($salvar->getResultado()) {
            $_SESSION['_zion_firewall'] = hash('sha512', random_int(100, 5000));
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-tamanhos/index&sucesso=true&token={$_SESSION['timeWT']}");
        } else {
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-tamanhos/index&erro=true&token={$_SESSION['timeWT']}");
        }
    }
