<div class="main-content">
    ...
    if (isset($criar['sendZion'])) {
        ...
        if ($criar['zion_firewall'] != $_SESSION['_zion_firewall']) {
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-paginas/index&erro=true&token={$_SESSION['timeWT']}");
            exit();
        }

        $salvar = new Posts();
        $salvar->atualizaPost($criar['id'], $criar);
        if ($salvar->getResultado()) {
            $_SESSION['_zion_firewall'] = hash('sha512', random_int(100, 5000));
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-paginas/index&sucesso=true&token={$_SESSION['timeWT']}");
        } else {
            header("Location: " . URL_CAMINHO_PAINEL . FILTROS . "zion-paginas/index&erro=true&token={$_SESSION['timeWT']}");
        }
    }
