...
require_once('zion-filtros/valida.php');
 ...
 if(isset($atualizar['sendZion'])){
   ...
   if($atualizar['zion_firewall'] != $_SESSION['_zion_firewall']){
    header("Location: " . URL_CAMINHO_PAINEL_CLIENTE . FILTROS ."zion-usuarios/index&erro=true&token=".$_SESSION['timeWT']); 
    exit();
   }
   ...
   if($salvar->getResultado()){
    $_SESSION['_zion_firewall'] = hash('sha512',  random_int(100, 5000));
     header("Location: " . URL_CAMINHO_PAINEL_CLIENTE . FILTROS ."zion-usuarios/index&sucesso=true&token=".$_SESSION['timeWT'] );
   }else{
     header("Location: " . URL_CAMINHO_PAINEL_CLIENTE . FILTROS ."zion-usuarios/index&erro=true&token=".$_SESSION['timeWT'] );
   }
